    ...
    <div class="p-4 h-full flex flex-col min-h-[200px]">
      ...
      <div class="flex items-center justify-between mb-3 text-sm">
        <div class="flex items-center text-gray-600">
          <UIcon name="i-heroicons-calendar-days" class="mr-1.5 text-gray-400 flex-shrink-0" size="16" />
        ...
        <div class="flex items-center text-gray-600 ml-2">
          <UIcon name="i-heroicons-users" class="mr-1 text-gray-400 flex-shrink-0" size="16" />
      ...
      <div class="flex items-center justify-between mb-3">
        <div class="flex-1">
          <span v-if="!hasMultipleEvents(competition)" :class="getCategoryBadgeClass(competition.category)" class="text-sm">
            {{ translateCategory(competition.category) }}
          </span>
          <span v-else class="inline-flex items-center px-2 py-1 rounded-md text-sm font-medium bg-purple-100 text-purple-700">
            <UIcon name="i-heroicons-list-bullet" class="mr-1" size="14" />
            {{ competition.events.length }} дисциплин
          </span>
        </div>
        
        <div v-if="!hasMultipleEvents(competition) && competition.points" class="flex items-center text-amber-600 ml-2">
          <UIcon name="i-heroicons-star" class="mr-1 text-amber-500 flex-shrink-0" size="16" />
      ...
      <div class="flex-1 flex flex-col">
        ...
        <div v-if="hasMultipleEvents(competition)" class="border-t border-gray-100 pt-3 mb-3 flex-1">
          <div class="space-y-2 h-16 overflow-y-auto">
            <div v-for="event in competition.events.slice(0, 2)" :key="event.slug"
              class="flex items-center justify-between text-sm">
              <div class="flex items-center flex-1 min-w-0">
                <span :class="event.hasResults ? 'w-2 h-2 bg-green-500' : 'w-2 h-2 bg-gray-300'" 
              ...
              <div class="flex items-center space-x-2 text-gray-500 ml-2">
                <span v-if="event.points" class="flex items-center">
                  <UIcon name="i-heroicons-star" class="mr-0.5 text-amber-400" size="14" />
                  {{ event.points }}
                </span>
                <span class="flex items-center">
                  <UIcon name="i-heroicons-user" class="mr-0.5" size="14" />
                  {{ event.participantCount || 0 }}
                </span>
              </div>
            </div>
            <div v-if="competition.events.length > 2" class="text-sm text-gray-500 text-center pt-1">
        ...
        <div v-else class="flex-1 min-h-[4rem]"></div>
      </div>
